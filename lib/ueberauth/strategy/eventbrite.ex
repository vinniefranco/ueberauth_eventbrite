  ...
  def handle_callback!(%Plug.Conn{params: %{"code" => code}} = conn) do
    ...
    case Ueberauth.Strategy.Eventbrite.OAuth.get_token([code: code], opts) do
      ...
      {:error, %OAuth2.Error{reason: reason}} ->
        set_errors!(conn, [error("oauth2_error", reason)])
      {:error, %OAuth2.Response{body: %{"error" => err, "error_description" => err_description}}} ->
        set_errors!(conn, [error(err, err_description)])
    end
  end
